...
long CCash::Transfer(CMyAccount *ptSourceA, CMyAccount *ptDestA, double sum, CString ptNote)
{
...
long CCash::Income(CString inType, double sum, CString ptNote)
{
	fstream mycbc;
	mycbc.open("CCash.txt");
	balance += sum;
...
long CCash::Consume(CString conType, double sum, CString ptNote)
{
	fstream mycbc;
	mycbc.open("CCash.txt");
	balance -= sum;
	mycbc.seekg(0, ios::end);//将指针调到文件最后
	mycbc << "支出： " << GetTime() << "  支出类型：" << conType << "  支出金额：" << sum << "  备注：" << ptNote << "  余额：" << balance << endl;
	mycbc.close();
	return 0;
}

long CCash::Statistic(CString &bankofChinaaccount)
{
	fstream mycbc;
	mycbc.open("CCash.txt");
