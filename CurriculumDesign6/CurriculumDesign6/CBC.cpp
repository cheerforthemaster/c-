...
long CBC::Transfer(CMyAccount *ptSourceA, CMyAccount *ptDestA, double sum, CString ptNote)
{
...
long CBC::Income(CString inType, double sum, CString ptNote)
{
	fstream mycbc;
	mycbc.open("CBC.txt");
	balance += sum;
...
long CBC::Consume(CString conType, double sum, CString ptNote)
{
	fstream mycbc;
	mycbc.open("CBC.txt");
	balance -= sum;
...
long CBC::Statistic(CString &bankofChinaaccount)
{
	fstream mycbc;
	mycbc.open("CBC.txt");
