...
public:
	...
	E getMinEdge(int &one, int &two);//获取最带权最小的边,并删除该边,返回权重;
	E getMinEdge(int &one, int &two, int postion);
private:
...
template<class T, class E>
inline E graph<T, E>::getMinEdge(int & one, int & two)
{
	MinGraph();
	minGraph[0] = -1;
	E w;
...
template<class T, class E>
inline E graph<T, E>::getMinEdge(int & one, int & two, int postion)
{
	Edge<T, E>* temEgde = NodeTable[postion].adj;
	if (temEgde == NULL)
		return NULL;
	E w;
	w = temEgde->cost;
...
template<class T, class E>
inline bool graph<T, E>::MinGraph()
{
	if (minGraph == NULL)
	{
		minGraph = new int[vertexNum];
		for (int i = 0; i < vertexNum; i++)
			minGraph[i] = i;
	}
	return true;
