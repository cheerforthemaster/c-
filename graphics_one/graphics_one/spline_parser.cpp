...
void SplineParser::SaveBezier(ArgParser *args) {
  ...
  FILE *file;
  fopen_s(&file,args->output_bezier_file, "w");
  assert (file != NULL);
...
void SplineParser::SaveBSpline(ArgParser *args) {
  ...
  FILE *file;
  fopen_s(&file,args->output_bspline_file, "w");
  assert (file != NULL);
  fprintf (file, "num_splines %d\n", getNumSplines());
  for (int i = 0; i < getNumSplines(); i++) {
...
void SplineParser::SaveTriangles(ArgParser *args) {
  ...
  FILE *file;
  fopen_s(&file,args->output_file, "w");
  assert (file != NULL);
  mesh.Output(file);
  fclose(file);
}
...
int SplineParser::getToken(char token[MAX_PARSER_TOKEN_LENGTH]) {
  ...
  assert (file != NULL);
  int success = fscanf_s(file,"%s ",token,100);
  if (success == EOF) {
    token[0] = '\0';
    return 0;
  }
...
Vec3f SplineParser::readVec3f() {
  float x,y,z;
  int count = fscanf_s(file,"%f %f %f",&x,&y,&z);
  if (count != 3) {
...
Vec2f SplineParser::readVec2f() {
  float u,v;
  int count = fscanf_s(file,"%f %f",&u,&v);
  if (count != 2) {
...
int SplineParser::readInt() {
  int answer;
  int count = fscanf_s(file,"%d",&answer);
  if (count != 1) {
