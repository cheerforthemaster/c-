...
public:
	...
	bool getData(int i, T& x);									 //获取第i个元素的数据
	bool Get_Data(int i, T& x);                                  //从第一个开始依次获取下个，若i==1则重新从第一个开始
	bool Set_Data(T& x);                                         //与Get_Data()配合，从而重置相应位置的数据
	bool setData(int i, T x);									 //将第i个位置的元素数据重置为x
	bool Insert(int i, T x);									 //在第i个元素后面插入数据为x的节点
	bool Delete(int i, T& x);								     //删除第i个位置的节点，并返回数据
...
protected:
	...
	list_node<T> *prt_num;                                       //获取数据的暂用节点
	list_node<T> *prt_num_last;                                  //重置数据的暂用节点
};
...
template<class T>
bool link_list<T>::getData(int i, T & x)                     //获取第i个元素的数据
{
...
template<class T>
bool link_list<T>::Delete(int i, T & x)                     //删除第i个位置的节点，并返回数据
{
	list_node<T> *prt = Loacte(i);
